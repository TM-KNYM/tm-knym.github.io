...
*/
	...
	function startDrop(egh) {
		...
		egh.position.z = -20000; // Math.round(9000 * (Math.random() - 0.5) / STEP) ;
		egh.position.y = 0;//STEP * Math.round(9000 * (Math.random() - 0.5) / STEP) ;
		egh.updateMatrix();
		...
		TweenMax.to(egh.position, sec, {
			...
			y: Math.round(9000 * (Math.random() - 0.5) / STEP),
			z: 0,
			ease: Expo.easeOut,
			onComplete: endDrop,
